...
# Phase D cross-build sysroot (static-musl userspace deps: libffi etc.); built by
# the scripts/crossbuild_deps.sh recipe into /home/timo/crossmusl/sysroot.
CROSSROOT="${CROSSROOT:-/home/timo/crossmusl/sysroot}"
mkdir -p "$OUT"
...
if command -v musl-gcc >/dev/null 2>&1; then
    musl-gcc -static -no-pie -O2 -ffunction-sections -fdata-sections -Wl,--gc-sections -s -I"$CROSSROOT/include" -o "$OUT/uhello" "$ROOT/tools/u_wl.c" "$CROSSROOT/lib/libwayland-client.a" "$CROSSROOT/lib/libwayland-util.a" "$CROSSROOT/lib/libffi.a" -lm
