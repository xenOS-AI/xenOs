...
BASE=(-drive file="$IMG",format=raw -drive file=build/fat.img,format=raw,if=ide,index=2 -device ich9-ahci,id=ahci -drive file=build/sata.img,format=raw,if=none,id=sd0 -device ide-hd,drive=sd0,bus=ahci.0 -drive file=build/rootfs.ext4,format=raw,if=ide,index=3 -m 1024 -machine pc -vga std -no-reboot \
